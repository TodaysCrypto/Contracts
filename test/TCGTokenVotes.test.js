...
contract('TCGTokenVotes', function (accounts) {
  const [ holder, recipient, holderDelegatee, recipientDelegatee, other1, other2, other3 ] = accounts;

  const name = 'TodaysCrypto';
  const symbol = 'TCG';
  const version = '1';
  const supply = new BN('5000000000000000000000000000');

  beforeEach(async function () {
    this.token = await TCGToken.new(holder);

    // We get the chain id from the contract because Ganache (used for coverage) does not return the same chain id
    // from within the EVM as from the JSON RPC interface.
    // See https://github.com/trufflesuite/ganache-core/issues/515
    this.chainId = (await getChainId()).toString();
  });
...
//   });
  ...
  it('recent checkpoints', async function () {
    ...
    await this.token.transfer(other1, 1);
    await this.token.transfer(other2, 1);
    await this.token.transfer(other3, 1);

    const block = await web3.eth.getBlockNumber();
    expect(await this.token.numCheckpoints(holder)).to.be.bignumber.equal('4');
    // recent
    expect(await this.token.getPastVotes(holder, block - 1)).to.be.bignumber.equal(supply.sub(new BN('2')));
    // non-recent
    expect(await this.token.getPastVotes(holder, block - 4)).to.be.bignumber.equal('0');
  });
  ...
  describe('set delegation', function () {
    describe('call', function () {
      ...
      it('delegation without balance', async function () {
        ...
        const { receipt } = await this.token.delegate(other1, { from: other1 });
        expectEvent(receipt, 'DelegateChanged', {
          delegator: other1,
          fromDelegate: ZERO_ADDRESS,
          toDelegate: other1,
        ...
        expect(await this.token.delegates(other1)).to.be.equal(other1);
      });
    });
    ...
    describe('with signature', function () {
      ...
      beforeEach(async function () {
        await this.token.transfer(delegatorAddress, supply);
      });
  ...
  describe('Compound test suite', function () {
    ...
    describe('getPastVotes', function () {
      ...
      it('returns the latest block if >= last checkpoint block', async function () {
        ...
        expect(await this.token.getPastVotes(other1, t1.receipt.blockNumber)).to.be.bignumber.equal(supply);
        expect(await this.token.getPastVotes(other1, t1.receipt.blockNumber + 1)).to.be.bignumber.equal(supply);
      });
      ...
      it('generally returns the voting balance at the appropriate checkpoint', async function () {
        ...
        expect(await this.token.getPastVotes(other1, t1.receipt.blockNumber)).to.be.bignumber.equal(supply);
        expect(await this.token.getPastVotes(other1, t1.receipt.blockNumber + 1)).to.be.bignumber.equal(supply);
        expect(await this.token.getPastVotes(other1, t2.receipt.blockNumber)).to.be.bignumber.equal(supply.sub(new BN(10)));
        expect(await this.token.getPastVotes(other1, t2.receipt.blockNumber + 1)).to.be.bignumber.equal(supply.sub(new BN(10)));
        expect(await this.token.getPastVotes(other1, t3.receipt.blockNumber)).to.be.bignumber.equal(supply.sub(new BN(20)));
        expect(await this.token.getPastVotes(other1, t3.receipt.blockNumber + 1)).to.be.bignumber.equal(supply.sub(new BN(20)));
        expect(await this.token.getPastVotes(other1, t4.receipt.blockNumber)).to.be.bignumber.equal(supply);
        expect(await this.token.getPastVotes(other1, t4.receipt.blockNumber + 1)).to.be.bignumber.equal(supply);
      });
    });
  });
  ...
  describe('getPastTotalSupply', function () {
    ...
    it('returns the latest block if >= last checkpoint block', async function () {
      t1 = await this.token.transfer(other1, supply);
    ...
    it('returns zero if < first checkpoint block', async function () {
      await time.advanceBlock();
      const t1 = await this.token.transfer(holder, supply);
      await time.advanceBlock();
      await time.advanceBlock();
    ...
    it('generally returns the voting balance at the appropriate checkpoint', async function () {
      ...
      await time.advanceBlock();
      await time.advanceBlock();
      const t2 = await this.token.transfer(other1, 10);
      await time.advanceBlock();
      await time.advanceBlock();
      const t3 = await this.token.transfer(other1, 10);
      await time.advanceBlock();
      await time.advanceBlock();
      const t4 = await this.token.transfer(other3, 20);
      await time.advanceBlock();
      await time.advanceBlock();
      ...
      expect(await this.token.getPastTotalSupply(t1.receipt.blockNumber)).to.be.bignumber.equal(supply);
      expect(await this.token.getPastTotalSupply(t1.receipt.blockNumber + 1)).to.be.bignumber.equal(supply);
      expect(await this.token.getPastTotalSupply(t2.receipt.blockNumber)).to.be.bignumber.equal(supply);
      expect(await this.token.getPastTotalSupply(t2.receipt.blockNumber + 1)).to.be.bignumber.equal(supply);
      expect(await this.token.getPastTotalSupply(t3.receipt.blockNumber)).to.be.bignumber.equal(supply);
      expect(await this.token.getPastTotalSupply(t3.receipt.blockNumber + 1)).to.be.bignumber.equal(supply);
      expect(await this.token.getPastTotalSupply(t4.receipt.blockNumber)).to.be.bignumber.equal(supply);
      expect(await this.token.getPastTotalSupply(t4.receipt.blockNumber + 1)).to.be.bignumber.equal(supply);
    });
  });
});