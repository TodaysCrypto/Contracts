...
function shouldBehaveLikeVesting(beneficiary) {
  it('check vesting schedule', async function () {
    ...
    for (const timestamp of this.schedule) {
      ...
      expect(await this.tcgVestingWallet.methods[vestedAmount](...args, timestamp)).to.be.bignumber.equal(vesting);

      expect(await this.tcgVestingWallet.methods[releasable](...args)).to.be.bignumber.equal(vesting);
    }
  });
  ...
  it('execute vesting schedule', async function () {
    ...
    {
      const receipt = await this.tcgVestingWallet.methods[release](...args);

      await expectEvent.inTransaction(receipt.tx, this.tcgVestingWallet, ...releasedEvent(this.token, '0'));
    ...
    for (const timestamp of this.schedule) {
      ...
      const receipt = await this.tcgVestingWallet.methods[release](...args);
      await expectEvent.inTransaction(receipt.tx, this.tcgVestingWallet, ...releasedEvent(this.token, vested.sub(released)));
